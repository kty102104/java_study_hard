...
/*
...

    {
        ...
        mvn -> Maven Repository에서 Gson 검색 -> version 클릭 -> (현재 저희 프로젝트 기준으로) Gradle(short) 클릭
        -> 복사한 후에 build.gradle 파일의 dependencies의 가장 밑에 붙여넣기 -> 우측 상단에 뜨는 파란색 코끼리 꼭 눌러야 합니다.
        JsonObject jsonObject = new JsonObject(); 했을 때 빨간줄 뜨면 import class를 통해서 import 문이 상단에 드는 것을
        확인하시면 됩니다. 안 될 경우 alt + enter로 불러올 수 있습니다.
...
public class JSON1 {
    public static void main(String[] args) {
        ...
        // {"username":"geunsu","password":1234,"email":"maybeags@gmail.com","name":"안근수"}
        // Map과의 차이점을 봤을 때 Map에서는 key 부분에 ""가 없었던 반면에
        ...
        /*
            Gson 라이브러리 :
                목적 : Gson 라이브러리는 Java 객체를 JSON 형태로 반환하거나,
                    그 반대로 변환하는데 사용
                주요 클래스
